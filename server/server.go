...
func (s *Server) v1QueryGet(w http.ResponseWriter, r *http.Request) {
	...
	results, err := s.execQuery(ctx, r, txn, decisionID, parsedQuery, nil, m, explainMode, includeMetrics, includeInstrumentation, pretty, nil)
	if err != nil {
		switch err := err.(type) {
		case ast.Errors:
...
func readInputCompilePostV1(r io.ReadCloser) (*compileRequest, *types.ErrorV1) {
	...
	if request.Schema != nil {
		schema, err = ast.InterfaceToValue(*request.Schema)
		if err != nil {
			return nil, types.NewErrorV1(types.CodeInvalidParameter, "error(s) occurred while converting schema: %v", err)
		}
	}
