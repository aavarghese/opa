...
func loadRego(path string, bs []byte, m metrics.Metrics, processAnnotation ...bool) (*RegoFile, error) {
	m.Timer(metrics.RegoModuleParse).Start()
	var module *ast.Module
	var err error
	if len(processAnnotation) == 1 {
		module, err = ast.ParseModule(path, string(bs), processAnnotation[0])
	} else {
